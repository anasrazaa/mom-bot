...
export default function Dashboard() {
  ...
  async function startMeeting(e) {
    ...
    setCreating(true);
    try {
      const mtg = await api.post('/meetings', {
        title: form.title,
      ...
      navigate('active-meeting', mtg.id);
    } catch (err) {
  ...
  return (
    <>
      ...
      <div className="stats-grid">
        <StatCard val={total}      lbl="Total Meetings" />
          ...
          <form className="form-card" onSubmit={startMeeting}>
            <div className="form-group">
              ...
              <input
                ...
                placeholder="e.g. Faculty Board Meeting – May 2025"
                value={form.title}
        ...
        <div>
          <div className="card">
            ...
            {recent.length === 0 ? (
              ...
              recent.map(m => (
                <div key={m.id} className="mini-meeting">
                  <div>
                    <div className="mini-mtg-title">{m.title}</div>
                    <div className="mini-mtg-date">{fmtDate(m.created_at)}</div>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <StatusChip status={m.status} />
                    <button className="btn btn-ghost btn-sm" onClick={() => navigate('meeting-detail', m.id)}>Open</button>
                  </div>
                </div>
              ))
            )}
